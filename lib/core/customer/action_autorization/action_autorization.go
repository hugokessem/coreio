...
type ActionAutorizationDetail struct {
	CustomerNumber         string
...
type ActionAutorizationResult struct {
	Success  bool
	Detail   *ActionAutorizationDetail
	Messages []string
}

func ParseActionAutorizationSOAP(xmlData string) (*ActionAutorizationResult, error) {
	var env Envelope
	...
	if env.Body.Fault != nil {
		...
		return &ActionAutorizationResult{
			Success:  false,
	...
	if strings.ToLower(resp.Status.SuccessIndicator) != "success" {
		...
		if len(messages) == 0 {
			messages = []string{"Action authorization failed"}
		}
		return &ActionAutorizationResult{
			Success:  false,
			Messages: messages,
