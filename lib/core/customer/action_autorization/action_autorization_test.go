...
func TestNewActionAutorization(t *testing.T) {
	xmlRequest := NewActionAutorization(Param{
...
func TestParseActionAutorizationSOAP_Success(t *testing.T) {
	...
	d := result.Detail
	assert.Equal(t, "1277588454", d.CustomerNumber)
	assert.Equal(t, "1277588454", d.TransactionID)
	...
	assert.Equal(t, []string{"KEBELEID/CUS*100 FROM 1277588454 NOT RECEIVED"}, d.Override)
	assert.Equal(t, "IDEL", d.RecordStatus)
	assert.Equal(t, "1", d.CurrNo)
	assert.Equal(t, "2878_SUPERAPP.A__OFS_GCS", d.Inputter)
	assert.Equal(t, "2609251528", d.DateTime)
