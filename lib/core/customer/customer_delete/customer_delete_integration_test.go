...
const deleteCustomerEndpoint = "https://devapisuperapp.cbe.com.et/superapp/parser/proxy/IIBONBOARDING/services?target=http%3A%2F%2F172.31.6.115%3A9095&wsdl=null"

func TestIntegrationDeleteCustomer(t *testing.T) {
	params := Param{
	...
	xmlRequest := NewCustomerDelete(params)
	...
	req, err := http.NewRequestWithContext(ctx, http.MethodPost, deleteCustomerEndpoint, strings.NewReader(xmlRequest))
	...
	result, err := ParseCustomerDeleteSOAP(string(responseData))
	...
	if result.Success {
		...
		t.Logf("Deleted customer: %s recordStatus=%s", result.Detail.CustomerNumber, result.Detail.RecordStatus)
		return
	}

	require.NotEmpty(t, result.Messages, "failed delete should return messages")
	t.Logf("Customer delete was not successful. Messages: %v", result.Messages)
}
